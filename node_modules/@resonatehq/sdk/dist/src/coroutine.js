...
class Coroutine {
    ...
    constructor(ctx, task, verbose, decorator, handler, spans, depth = 1) {
        ...
        this.spans = spans;
        this.depth = depth;
        if (!(this.ctx.retryPolicy instanceof retries_1.Never) && !logged.has(this.ctx.id)) {
    ...
    static exec(id, verbose, ctx, func, args, task, handler, spans, callback) {
        handler.createPromise({
            ...
            const coroutine = new Coroutine(ctx, task, verbose, new decorator_1.Decorator(func(ctx, ...args)), handler, spans);
        ...
        const next = () => {
            while (true) {
                ...
                if (action.type === "internal.async.l") {
                    ...
                    this.handler.createPromise(action.createReq, (err, res) => {
                        ...
                        if (res.state === "pending") {
                            ...
                            spans.push(span);
                            const coroutine = new Coroutine(ctx, this.task, this.verbose, new decorator_1.Decorator(action.func(ctx, ...action.args)), this.handler, this.spans, this.depth + 1);
                            const cb = (err, status) => {
                                if (err) {
                                    span.end(this.ctx.clock.now());
                                    return callback(err);
                                    ...
                                    this.handler.completePromise({
                                        ...
                                        input = {
                                            ...
                                            value: {
                                                ...
                                                value: res.state === "resolved" ? (0, types_1.ok)(res.value?.data) : (0, types_1.ko)(res.value?.data),
                                            },
                                        };
                                        next();
                                    }, action.func.name);
                ...
                if (action.type === "internal.await" && action.promise.state === "completed") {
                    util.assert(action.promise.value.type === "internal.literal", "promise value must be an 'internal.literal' type");
